...
/**
 * REST controller for managing {@link uk.ac.bham.teamproject.domain.Item}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class ItemResource {
        ...
        Optional<Item> result = itemRepository
            ...
            .map(existingItem -> {
                ...
                if (item.getPostcode() != null) {
                    existingItem.setPostcode(item.getPostcode());
                }
                if (item.getUploadTime() != null) {
                    existingItem.setUploadTime(item.getUploadTime());
                }
                if (item.getGivenTime() != null) {
                    existingItem.setGivenTime(item.getGivenTime());
                }
     ...
     */
    @GetMapping("/items")
    public List<Item> getAllItems(@RequestParam(required = false, defaultValue = "false") boolean eagerload) {
